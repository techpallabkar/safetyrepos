...
class PostMasterBean extends MasterBean {
	public function post_validation() {
		...
		if( $status_id == 2 ) {
			$suggested_category_id 	= isset( $data["suggested_category_id"] ) ? trim( $data["suggested_category_id"] ) : 0;
			...
			//if( $suggested_category_id == "" || ! $suggested_category_id ) {
			//	$this->error_messages[ "suggested_category_id_error" ] = "<div class=\"errors\">Suggested category is required.</div>";
			//	$error_counter++;
			//}
			if( $author == "" ) {
