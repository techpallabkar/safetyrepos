...
class LibraryMasterDAO extends MainDao {
    ...
    public $_table_library = "library";
    public $_view_library = "master_library_categories_view";

    public function __construct() {
        parent::__construct();
    }

    public function getIdsfrommappingtable($selectval, $table, $selection) {
        $query = "SELECT $selection FROM $table WHERE tree_division_id LIKE '$selectval%' ";
        $rows = $this->select($query);
        return $rows;
    }

    public function getalldatafromview($finalquerypart, $from_date, $to_date, $col = null, $peninginvestigation = null, $paction = null, $colname, $activityId, $table, $activityclaue) {
        if ($from_date != "" && $to_date != "") {
            $between = "AND $colname BETWEEN '" . $from_date . "' AND '" . $to_date . "'";
        }
        if ($from_date != "" && $to_date == "") {
            $between = "AND $colname BETWEEN '" . $from_date . "' AND '" . $from_date . "'";
        }
        if ($from_date == "" && $to_date != "") {
            $between = "AND $colname BETWEEN '" . $to_date . "' AND '" . $to_date . "'";
        }
        if ($from_date == "" && $to_date == "") {
            $between = "";
        }
        if ($activityId) {
            $clause = " AND $activityclaue = $activityId";
            $caluse2 = $between;
        } else {
            $clause = "";
            $caluse2 = "";
        ...
        if ($col != "") {
            $colvalue = " AND job_stop_req_raised='Y'";
        } else {
            $colvalue = "";
        }

        if ($peninginvestigation != "") {
            $colvalue2 = " AND investigation_req='Y' AND investigation_done='N'";
        } else {
            $colvalue2 = "";
        }

        if ($paction != "") {
            $colvalue3 = " AND no_of_finding > 0 AND investigation_done='Y'";
        } else {
            $colvalue3 = "";
        }
        if ($activityId == 7) {
            $colvalue4 = " AND no_of_deviation > 0 AND major_deviation ='Y'";
        } else {
            $colvalue4 = "";
        }
        $query = "SELECT * FROM $table WHERE id IN($finalquerypart) and activity_type_id != '3' and status_id='3' $clause $caluse2 $colvalue $colvalue2 $colvalue3 $colvalue4 ORDER BY  activity_type_id";
//        show($query);
        $rows = $this->select($query);
        return $rows;
    }
...
//                $colvalue3 = "";
//            }
//            $query = "SELECT * FROM $table WHERE id IN($finalquerypart) and status_id='3' $clause $caluse2 $colvalue $colvalue2 $colvalue3 ORDER BY  activity_type_id";//echo $query."<br>";
//            echo $query;
//            $rows = $this->select($query);
//            return $rows ;
//        }

    public function getalldatafromsafobsview($finalquerypart, $from_date, $to_date, $activityId, $table, $activityclaue) {
...
//            $to_date_year = date("Y",strtotime($to_date));
        ...
        if ($activityId) {
            $clause = " AND $activityclaue = $activityId";
            $caluse2 = " AND CONCAT((activity_year),'-', (lpad(activity_month,2,'0')),'-01') BETWEEN '" . $from_date . "' AND '" . $to_date . "' ";
        } else {
            $clause = "";
            $caluse2 = "";
        }

        $query = "SELECT * FROM $table WHERE id IN($finalquerypart) AND status_id='3' $clause $caluse2 ORDER BY  activity_type_id";
    ...
    public function get_activity_type_master($clause = "") {
        $clause = ( $clause == "" ) ? 1 : $clause;
        $query = "SELECT * FROM activity_type_master WHERE " . $clause . " ORDER BY orderID ASC";
        return $this->select($query);
    }
...
//    public function get_inc_addressed($clause = null, $tree_id,$pval) {show($pval);
    public function get_inc_addressed($clause = null, $pval) {//show($pval);
        $rowclause = "";
        if ($clause != "") {
            $rowclause = $clause;
        }
        $query = "SELECT t1.* ,t2.date_of_incident, count( t1.id ) as totalcount, t3.tree_division_id
                        FROM `incident_finding_mapping` t1 
                        LEFT JOIN incident t2 ON t2.id = t1.incident_id 
                        LEFT JOIN incident_division_mapping t3 ON t3.incident_id = t2.id 
                        WHERE tree_division_id LIKE CONCAT('%', :tree_division_id, '%') AND t1.incident_id!=:incident_id " . $rowclause . " 
                        GROUP BY t1.`incident_id`,t1.`action_taken`";
        $resutl = $this->db->prepare($query);
        $resutl->execute($pval);
        return $resutl->fetchAll();
    }

    public function get_inc_details_by_id($id) {
        $query = "SELECT t1.* , count( t1.id ) as totalcount
                    FROM `incident_finding_mapping` t1 
        ...
        $returnvalue = 0;
        if (count($rowdata) > 0) {
            $returnvalue = $rowdata[0]->totalcount;
        }
        return ($returnvalue);
    }

    public function get_inc_inv_pending($clause = null, $tree_id) {//show($tree_id);      
        $rowclause = "";
        if ($clause != "") {
            $rowclause = $clause;
        }
        $rowclause .= "AND tree_division_id LIKE '%$tree_id%'";
        $query = "SELECT id,date_of_incident,investigation_done
                    FROM actualtarget_incident_view 
                    WHERE investigation_done = 'N' 
                    AND created_by != '2' 
                    AND deleted = '0' " . $rowclause; 
        $rowdata = $this->select($query);
        return ($rowdata);
    }
    ...
    public function getLibraryCategories($clause = 1) {
        $clause = ( $clause == "" ) ? 1 : $clause;
        $query = "SELECT * FROM master_library_categories WHERE deleted = 0 AND " . $clause;
        $resutl = $this->db->prepare($query);
        $resutl->execute();
        return $resutl->fetchAll();
    }
    ...
    public function get_buyers_guides_with_pagging($where_clause = 1, $limit = 10, $auth_user = array()) {
        ...
        $where_clause = ( $where_clause == "" ) ? 1 : $where_clause;
        $start = ( $this->pagging->page > 1 ) ? ($this->pagging->limit * $this->pagging->page) - $this->pagging->limit : 0;
        $query = "SELECT COUNT(id) AS maxrow FROM `" . $this->_view_library . "` WHERE " . $where_clause;
        ...
        $rows = array();
        $query = "SELECT * FROM " . $this->_view_library . " WHERE " . $where_clause . " LIMIT " . $start . ", " . $this->pagging->limit;
        $rows = $this->select($query);
