...
<div class="container1">
    ...
    <h1 class="heading" style="border-bottom:3px solid #ff9f08;">20.[Graph] Statics for MRM</h1> 
    ...
    <div class="print-friendly" id="divToPrint" style="padding-bottom:50px;">
    <!--<div id='TBL_EXPORT'>-->
                ...
                <thead class="bg-primary">    
                    <tr>
                        ...
                        foreach ($get_statistics_data["CUR_YEAR_YTM"] as $key2 => $value2) {
                            ...
                            echo '<th>YTM[' . (date("Y", strtotime($arr2[0]))) . "-" . (date("y", strtotime($arr2[0]))+1) . ']</th>';
                        }
                        ?>
                    </tr>
                </thead>
        ...
        <div id="chart">
            <ul id="bars">
                ...
                foreach ($get_statistics_data["CUR_YEAR_YTM"] as $key2 => $value2) {
                    $arr2 = explode(" AND ", $str2 = str_replace("'", "", $key2));
                    echo '<li><div data-percentage="' . ($value2) . '" class="bar"><span class="ptext">' . ($value2) . '</span></div><span>YTM(' . (date("Y", strtotime($arr2[0]))) . "-" . (date("y", strtotime($arr2[1]))+1) . ')</span></li>';
                }
                ?>
            </ul>
...
<script type="text/javascript">   
    ...
    mywindow.document.write('<style>#chart{display: block;margin: 30px 0 0 95px;border:1px solid #666;height:460px;font-size:12px !important;font-family:Arial !important;}#bars {margin-top:60px;}#chart #bars{display:table;}#chart #bars li {display: table-cell;height: 300px;margin: 0;position: relative;text-align: center;width: 58px;bottom:0 !important;}#chart #bars li span{position:absolute; bottom:0;left:7px;}#chart #bars{height:350px !important;display:block;width:100% !important;}#chart #bars li .bar{background: #001d3f !important; bottom: 40px;color: #000 !important;display: block;margin-left: 14px;position: absolute;width: 20px;-webkit-print-color-adjust: exact;color-adjust: exact;}span.ptext {color:#000 !important;-webkit-print-color-adjust: exact;color-adjust: exact;font-weight: bold;margin-bottom: -18px;margin-left: -5px;left:0 !important}table{border-collapse:collapse;margin-bottom:15px;}table th, table td {border:1px solid #80aaff;border-collapse:collapse;}table th {background: #88b5dd !important;-webkit-print-color-adjust: exact;color-adjust: exact;border:1px solid #609dd2;}</style></head><body>');
