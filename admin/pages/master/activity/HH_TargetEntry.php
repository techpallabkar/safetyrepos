...
<div class="container1">
    ...
    <form action="" method="post" id="fyr" enctype="multipart/form-data">
        ...
        <div class="col-sm-4">
            ...
            <a href="HH_TargetEntry.php" class="btn btn-danger btn-sm">Reset</a>
        </div>
    </form>
    ...
    foreach ($getAllMonthofFinancialYear as $k => $rowdata) { 
                ...
                if($key == ($k + 1)) {
                    ...
                    foreach ($allDistrict as $ky => $row) { 
                        foreach ($arr1 as $y => $r) { 
                            if($r == 'H') {
                                echo '<td>'
                                 . '<input type="hidden" name="data['.$r."-".$row->id."-". ($k + 1) .']" value="0" style="width: 100%;"/>'
                                 . '<input type="checkbox" name="data['.$r."-".$row->id."-". ($k + 1) .']" value="1"'. ((($array[$key][$allDistrict[$ky]->id][$r]) == 1) ? "checked" : "") .' style="width: 100%;"/>'
                                 .  '</td>';
                            } else {
                    ...
                    $(document).ready(function () { 
                        $("#id1<?php echo $allDistrict[$ky]->id.$r.$i ?>").keydown( function () { 
                              ...
                              var gworkmanp = (($('#id1<?php echo $allDistrict[$ky]->id.$r.$i ?>').val() == '') ? 0 : parseInt($('#id1<?php echo $allDistrict[$ky]->id.$r.$i ?>').val()));
                        ...
                        $("#id2<?php echo $allDistrict[$ky]->id.$r.$i ?>").keydown( function () {

                              var csetval = (($('#id2<?php echo $allDistrict[$ky]->id.$r.$i ?>').val() == '') ? 0 : parseInt($('#id2<?php echo $allDistrict[$ky]->id.$r.$i ?>').val()));
                        ...
                        $("#id2<?php echo $allDistrict[$ky]->id.$r.$i ?>").keyup( function () {
     ...
     <?php 
            ...
            foreach ($allDistrict as $key => $value) {
                echo '<td>'
                . '<input type="hidden" name="data['."H-".$value->id."-". ($k + 1) .']" value="0" style="width: 100%;"/>'
                . '<input type="checkbox" name="data['."H-".$value->id."-". ($k + 1) .']" value="1" style="width: 100%;"/>'
                . '</td>';
                echo '<td><input type="text" name="data['."P-".$value->id."-". ($k + 1) .']" value="" style="width: 100%;" id="pid1'.$k.$value->id.$i.'"/></td>';
                echo '<td><input type="text" name="data['."C-".$value->id."-". ($k + 1) .']" value="" style="width: 100%;" id="cid2'.$k.$value->id.$i.'"/></td>';
             ?>
                
            <script type="text/javascript">
                    $(document).ready(function () {
                        
                      $("#pid1<?php echo $k.$value->id.$i ?>").keydown( function () {

                            var gworkmanp = (($('#pid1<?php echo $k.$value->id.$i ?>').val() == '') ? 0 : parseInt($('#pid1<?php echo $k.$value->id.$i ?>').val()));
                      ...
                      $("#pid1<?php echo $k.$value->id.$i ?>").keyup( function () {
                      ...
                      $("#cid2<?php echo $k.$value->id.$i ?>").keydown( function () {

                            var csetval = (($('#cid2<?php echo $k.$value->id.$i ?>').val() == '') ? 0 : parseInt($('#cid2<?php echo $k.$value->id.$i ?>').val()));
                      ...
                      $("#cid2<?php echo $k.$value->id.$i ?>").keyup( function () {
