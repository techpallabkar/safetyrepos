                    ...
                    if (count($devition_names) > 0) {
                        if (!empty($devition_names)) {
                            ...
                            foreach ($devition_names as $key => $ddmrow) {
                                ...
                                echo $j . '. <b>' . $ddmrow . '</b>&nbsp;&nbsp;';
                                ?>
                                <a href="edit_audit_nm.php?action=delete_division&t=<?php echo base64_encode("audit_division_mapping"); ?>&id=<?php echo $activity_id; ?>&delid=<?php echo $key; ?>" onclick="return confirm('Are you sure want to delete this?');">
		...
		echo '</select>'
            ...
            <div class="holder required">
                ...
                <?php
                    ...
                    ?>
                <select type="text" name="audit_by[]" id="audit_by" multiple style="width:300px;height:180px;" />
                    <?php
            ...
            <div class="holder">
                ...
                <input type="text"  name="data[sas_report_no]" id="sas_report_no" value="<?php echo $beanUi->get_view_data("sas_report_no"); ?>" />
                ...
                foreach ($post_uploads as $purow) {
                    ...
                    if ($type_id == 1 && is_file(CESC_BASE_PATH . "/" . $purow->file_path)) {
                        ...
                        if (!in_array($status_id, array(3, 5))) {
                            $pdf_links .= '<a class="btn btn-danger btn-sm" href="edit_audit_nm.php?action=delete_upload&t=' . base64_encode("file_upload_audit_mapping") . '&id=' . $purow->id . '&post_id=' . $activity_id . '" title="Delete file" onclick="return confirm(\'Confirm delete.\');">
                        ...
                        if ($status_id == 1) {
                            $image_links .= '<a href="edit_audit_nm.php?action=delete_upload&t=' . base64_encode("file_upload_audit_mapping") . '&id=' . $purow->id . '&post_id=' . $activity_id . '" 
                        ...
                        if ($status_id == 1) {
                            $video_links .= '<a href="edit_audit_nm.php?action=delete_upload&t=' . base64_encode("file_upload_audit_mapping") . '&id=' . $purow->id . '&post_id=' . $activity_id . '" 
                                title="Delete image" onclick="return confirm(\'Confirm delete.\');" class="button" style="background-color: #ef0000;border-radius: 4px;color: #fff;margin: 0;padding: 3px 10px 11px;position: relative;top: 1px;">
                                Delete</a>';
                        }
                        $video_links .= "</div>\n";
            ...
            <div class="holder">
                <fieldset class="">
                    ...
                    if ($beanUi->get_view_data("featured_image_path") != '' && file_exists($imgpath)) {
                        ...
                        $actions = '<a class="btn btn-sm btn-danger" href="edit_audit_nm.php?action=delete_featured_image&t=' . base64_encode("audit") . '&id=' . $activity_id . '"  onclick="return confirm(\'Confirm delete.\');"><i class="fa fa-trash"></i> Delete Featured Image</a>';
                    } else {
...
<script type="text/javascript">
    $(function () {
        ...
        $.ajax({
            type: 'post', url: '<?php echo page_link("activity/edit_audit_nm.php"); ?>', cache: false,
            data: ajax_data,
            success: function (data) {
                if (data)
                {
